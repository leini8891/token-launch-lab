...
async function renderDashboard() {
    ...
    const artifactDescriptions = new Map([
  ...
  <main>
    ...
    <section class="grid">
      ...
      <article class="panel">
        ...
        <div class="artifacts">
          ${artifacts
        .map((file) => `<div class="artifact-card"><strong>${escapeHtml(file)}</strong><p>${escapeHtml(artifactDescriptions.get(file) ?? "Generated launch artifact")}</p></div>`)
        .join("")}
        </div>
      </article>
    </section>
    ...
    <section class="panel">
      ...
      <div class="timeline">
        ${timelineRows
        .map((row) => `<div class="timeline-row"><span class="mono">${escapeHtml(row.timestamp.split("T")[1]?.replace("Z", "") ?? row.timestamp)}</span><strong>${escapeHtml(row.agent)}</strong><span class="outcome">${escapeHtml(row.outcome)}</span><span>${escapeHtml(`${row.action} ${row.note}`.trim())}</span></div>`)
        .join("")}
      </div>
    </section>
  </main>
